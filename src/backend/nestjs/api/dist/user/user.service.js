...
const postgres_error_codes_1 = require("@drdgvhbh/postgres-error-codes");
let UserService = class UserService {
    ...
    async getCurrentUser(req) {
        ...
        try {
            const user = await this.usersRepository.findOneBy({ email: reqUser.email });
            return user;
        }
        catch (err) {
            throw err;
    ...
    async createUser(user) {
        try {
            ...
            if (err.code == postgres_error_codes_1.PG_UNIQUE_VIOLATION)
