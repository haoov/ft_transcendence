<template>
	<div class="header-channel">
		<div class="profile-img-div" >
			<img v-if="mode === 'Private'" :src="channelImage" alt="Profile Picture">
		</div>
		<p>{{ channelName }}</p>
		<div class="logos-div">
			<div class="logo" @click="openEditChannelForm()" v-if="channel && isAdmin && !isPrivate" title="Edit channel">
				<svg
				fill="#b8b7b7"
				height="64px"
				width="64px"
		...
		<div class="logo" @click="opendAddUserForm()" v-if="channel && !isPrivate" title="Add Users">
			<svg
			width="64px"
			height="64px"
			viewBox="0 0 20 20"
		...
		<div class="logo" @click="leaveChannel()" v-if="channel && !isPrivate" title="Leave channel">
...
onMounted(() => {
	console.log(`Channel Name -> ${chat.getCurrentChannel()?.getName()}`);
});
...
const mode = channel.value?.getMode();
const channelImage = computed(() => {
	if (!channel.value) return undefined;
	if (channel.value.getMode() === 'Private') {
		const id1 : number = parseInt(channel.value.getName().split('#')[1]);
		const id2 : number = parseInt(channel.value.getName().split('#')[2]);
		if(id1 == currentUser.value.id) {
			const userFind = channel.value.getUsers().find((user: User) => user.id === id2)
			return userFind?.avatar;
		}
		const userFind = channel.value.getUsers().find((user: User) => user.id === id1)
		return userFind?.avatar;
	}
});
const channelName = computed(() => {
	...
	if (channel.value.getMode() === 'Private') {
		...
		const userFind = channel.value.getUsers().find((user: User) => user.id === id1)
		return userFind?.username;
	} else if (channel.value.getName().length > 10) {
		return channel.value.getName().slice(0, 7) + "...";
	}
	return channel.value.getName();
});
...
const isAdmin = computed( async() => {
	...
	channel.value.loadAdmins();
	const admins = channel.value.getAdmins();
	if (channel.value.getCreatorId() === currentUser.value.id
